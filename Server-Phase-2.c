...
void Login()
{
    ...
    else
    {
        ...
        cJSON *dialoges = cJSON_Parse(temp);
        cJSON *password = cJSON_GetObjectItemCaseSensitive(dialoges, "password");

        //If username exists, then check password
        if (strcmp(secondparameter, password->valuestring) == 0)
        {
            fclose(p);
...
void Refresh()
{
    ...
    cJSON *a = cJSON_Parse(temp);
    cJSON *dialoges = cJSON_GetObjectItemCaseSensitive(a, "messages");
    ...
    for (int i = onlinechannel[n].members[onlinechannel[n].membercounter - 1].refreshnumber; i < cJSON_GetArraySize(dialoges); i++)
    {
        cJSON *item = cJSON_GetArrayItem(dialoges, i);
        cJSON *sender = cJSON_GetObjectItemCaseSensitive(item, "sender");
        cJSON *message = cJSON_GetObjectItemCaseSensitive(item, "content");

        //Prepare content for send
        strcpy(list + strlen(list), "{\"sender\":\"");
        strcpy(list + strlen(list), sender->valuestring);
        strcpy(list + strlen(list), "\",");
        strcpy(list + strlen(list), "\"content\":\"");
        strcpy(list + strlen(list), message->valuestring);
        strcpy(list + strlen(list), "\"},");
    }

    //Relocate refresh of user number
    fclose(p);
    onlinechannel[n].members[onlinechannel[n].membercounter - 1].refreshnumber = cJSON_GetArraySize(dialoges);
